...
// ============================================================
// HERSHEY FONT DATA — built-in single-line CNC fonts
// [advanceWidth, [[x1,y1,x2,y2],...]]  cap-height = 21 units
// Hebrew glyphs are pre-mirrored for correct RTL rendering
// ============================================================
...
const SANS_DATA = {
  ...
  '-':[10,[[1,10,9,10]]],'.':[4,seg([[1,1],[1,0],[2,0],[2,1],[1,1]])],',':[4,seg([[2,2],[2,1],[3,1],[3,2],[2,2],[1,0]])],
  '/':[10,[[0,0,10,21]]],'\\':[10,[[0,21,10,0]]],':':[4,seg([[1,12],[1,11],[2,11],[2,12],[1,12],[1,2],[1,1],[2,1],[2,2],[1,2]])],
  '=':[12,[[1,13,11,13],[1,7,11,7]]],'+':[12,[[6,21,6,0],[1,10,11,10]]],
  '?':[10,seg([[1,16],[2,14],[4,12],[7,10],[9,8],[10,5],[10,3],[9,1],[7,0],[5,0],[3,1],[2,2],[9,21],[11,19],[11,16],[9,14],[7,13]])],'#':[12,[[3,21,3,-7],[9,21,9,-7],[0,14,12,14],[0,3,12,3]]],
  ...
  // Hebrew — pre-mirrored, correct RTL glyphs
  'א':[14,[[1,0,7,14],[7,14,13,0],[7,14,7,7]]],
  'ב':[13,[[0,14,10,14],[10,14,13,12],[13,12,13,1],[13,1,0,1],[13,7,6,7]]],
  'ג':[11,[[0,14,9,14],[9,14,11,12],[11,12,11,0]]],
  'ד':[13,[[13,0,13,12],[13,12,11,14],[11,14,0,14],[0,14,0,0]]],
  'ה':[13,[[13,0,13,14],[13,14,0,14],[0,14,0,5],[7,14,7,0]]],
  'ו':[5,[[2,14,2,0]]],
  'ז':[9,[[9,14,1,14],[1,14,4,0]]],
  'ח':[13,[[13,0,13,14],[0,0,0,14],[13,14,0,14]]],
  'ט':[13,[[13,0,13,14],[13,14,0,14],[0,14,0,0],[6,14,6,5]]],
  'י':[6,[[1,14,3,12],[3,12,3,0]]],
  'כ':[12,[[0,14,10,14],[10,14,12,12],[12,12,12,1],[12,1,0,1]]],
  'ך':[12,[[0,14,10,14],[10,14,12,12],[12,12,12,-5]]],
  'ל':[11,[[1,14,9,14],[9,14,11,12],[11,12,11,6],[11,6,9,4],[9,4,1,4],[1,4,1,0]]],
  'מ':[13,[[13,0,13,14],[0,0,0,10],[0,10,6,14],[6,14,13,14],[6,14,6,0]]],
  'ם':[13,[[13,0,13,14],[0,0,0,14],[13,14,0,14],[13,0,8,0]]],
  'נ':[11,[[1,14,9,14],[9,14,11,12],[11,12,11,0],[11,14,1,0]]],
  'ן':[5,[[3,14,3,-4]]],
  'ס':[13,seg([[6,14],[8,14],[10,13],[13,11],[13,5],[10,3],[8,2],[6,2],[4,2],[2,3],[0,5],[0,9],[2,11],[4,13],[6,14],[6,7,13,7]])],
  'ע':[13,[[13,14,9,7],[9,7,7,0],[7,0,4,7],[4,7,0,14],[9,7,4,7]]],
  'פ':[12,[[0,14,9,14],[9,14,12,12],[12,12,12,8],[12,8,9,6],[9,6,4,6],[4,6,4,0],[13,0,4,0]]],
  'ף':[12,[[0,14,9,14],[9,14,12,12],[12,12,12,8],[12,8,9,6],[9,6,4,6],[4,6,4,-5]]],
  'צ':[13,[[13,0,9,14],[9,14,5,0],[5,0,1,14],[9,14,1,14]]],
  'ץ':[13,[[13,0,9,14],[9,14,5,0],[5,0,1,14],[9,14,1,14],[5,0,5,-5]]],
  'ק':[12,[[12,0,12,14],[12,14,1,14],[1,14,1,5]]],
  'ר':[11,[[11,0,11,12],[11,12,9,14],[9,14,0,14]]],
  'ש':[14,[[14,0,14,14],[14,14,7,14],[7,14,7,0],[7,14,0,0]]],
  'ת':[14,[[14,0,14,14],[14,14,0,14],[0,14,0,5],[7,14,7,0]]],
};
...
function makeGothic(f){const o={...f};o['A']=[14,[[0,0,7,21],[14,0,7,21],[2,6,12,6],[0,0,2,0],[14,0,12,0]]];o['O']=[14,[[0,0,0,21],[0,21,14,21],[14,21,14,0],[14,0,0,0]]];o['E']=[12,[[0,0,0,21],[0,21,12,21],[0,11,9,11],[0,0,12,0]]];o['S']=[12,[[12,21,0,21],[0,21,0,11],[0,11,12,11],[12,11,12,0],[12,0,0,0]]];return o;}

const HERSHEY_FONTS = {
  sans:   {name:'Sans',   data: SANS_DATA},
  italic: {name:'Italic', data: makeItalic(SANS_DATA)},
  bold:   {name:'Bold',   data: makeBold(SANS_DATA)},
  gothic: {name:'Gothic', data: makeGothic(SANS_DATA)},
};
...
function hersheyToStrokes(text, fontData, sizeMm, spacingMm, lineHMm) {
  const scale = sizeMm / 21;
  const out = [];
  text.split('\n').forEach((line, li) => {
    ...
    chars.forEach(ch => {
      ...
      segs.forEach(([x1,y1,x2,y2]) => {
        out.push({
          x1: xOff + x1*scale, y1: li*lineHMm + y1*scale,
          x2: xOff + x2*scale, y2: li*lineHMm + y2*scale,
        });
      });
      xOff += w*scale + spacingMm;
    });
  });
  return out;
}
...
// ============================================================
// OPENTYPE.JS — TTF/OTF path → strokes
// Loaded from CDN, handles all fonts including Hebrew
// ============================================================
...
async function loadOpentype() {
  ...
  opentypeLib = window.opentype;
  return opentypeLib;
}

// Sample bezier curve to line segments
function sampleBezier(pts, steps=8) {
  const out = [];
  ...
  for (let i=1; i<=steps; i++) {
    ...
    // De Casteljau
    let p = [...pts];
    for (let j=0; j<n; j++) for (let k=0; k<n-j; k++) p[k] = {x:p[k].x*(1-t)+p[k+1].x*t, y:p[k].y*(1-t)+p[k+1].y*t};
    out.push({x1:prev.x, y1:prev.y, x2:p[0].x, y2:p[0].y});
    prev = p[0];
  }
  return out;
}

async function opentypeToStrokes(fontBuffer, text, sizeMm, spacingMm, lineHMm) {
  const ot = await loadOpentype();
  const font = ot.parse(fontBuffer);
  const UNITS = font.unitsPerEm;
  const scale = sizeMm / UNITS;
  const out = [];

  const lines = text.split('\n');
  lines.forEach((line, li) => {
    let xOff = 0;
    // Use opentype glyph run (handles RTL/Hebrew automatically)
    const glyphs = font.stringToGlyphs(line);
    glyphs.forEach(glyph => {
      const adv = (glyph.advanceWidth || UNITS*0.5) * scale;
      const path = glyph.getPath(0, 0, UNITS);
      // Extract contour centerlines from path commands
      let cx=0, cy=0, startX=0, startY=0;
      path.commands.forEach(cmd => {
        if (cmd.type==='M') { cx=cmd.x; cy=cmd.y; startX=cmd.x; startY=cmd.y; }
        else if (cmd.type==='L') {
          out.push({ x1:xOff+cx*scale, y1:li*lineHMm-cy*scale, x2:xOff+cmd.x*scale, y2:li*lineHMm-cmd.y*scale });
          cx=cmd.x; cy=cmd.y;
        }
        else if (cmd.type==='C') {
          const segs = sampleBezier([{x:cx,y:cy},{x:cmd.x1,y:cmd.y1},{x:cmd.x2,y:cmd.y2},{x:cmd.x,y:cmd.y}]);
          segs.forEach(s => out.push({ x1:xOff+s.x1*scale, y1:li*lineHMm-s.y1*scale, x2:xOff+s.x2*scale, y2:li*lineHMm-s.y2*scale }));
          cx=cmd.x; cy=cmd.y;
        }
        else if (cmd.type==='Q') {
          const segs = sampleBezier([{x:cx,y:cy},{x:cmd.x1,y:cmd.y1},{x:cmd.x,y:cmd.y}]);
          segs.forEach(s => out.push({ x1:xOff+s.x1*scale, y1:li*lineHMm-s.y1*scale, x2:xOff+s.x2*scale, y2:li*lineHMm-s.y2*scale }));
          cx=cmd.x; cy=cmd.y;
        }
        else if (cmd.type==='Z') {
          out.push({ x1:xOff+cx*scale, y1:li*lineHMm-cy*scale, x2:xOff+startX*scale, y2:li*lineHMm-startY*scale });
          cx=startX; cy=startY;
        }
      });
      xOff += adv + spacingMm;
...
const FONT_LIST = [
  ...
  'Courier New','Impact','Palatino','Garamond','Segoe UI','Calibri',
  'Cambria','Consolas','Helvetica','Comic Sans MS',
  // Hebrew fonts
  'David','Frank Ruehl','Miriam','Narkisim','Arial Hebrew',
  'Guttman Yad','Guttman Frank','Hadasim CLM','Alef',
];

function detectSystemFonts() {
  const cv = document.createElement('canvas');
  const ctx = cv.getContext('2d');
  const test = 'abcdefghij0123456789';
...
function drawCanvas(canvas,strokes){
  ...
  for(const s of strokes){
    if(prev&&Math.hypot(prev.x2-s.x1,prev.y2-s.y1)>0.05){
      ctx.strokeStyle='rgba(220,50,50,0.35)';ctx.lineWidth=0.8;ctx.setLineDash([3,3]);
...
export default function TextToGcode(){
  ...
  const [tab,setTab]=useState('hershey'); // hershey | system | ttf
  const [hKey,setHKey]=useState('sans');
  const [sysFonts,setSysFonts]=useState([]);
  const [sysFont,setSysFont]=useState('');
  const [ttfBuf,setTtfBuf]=useState(null);
  ...
  useEffect(()=>{
    const f=detectSystemFonts();
    setSysFonts(f);
    if(f.length)setSysFont(f[0]);
  },[]);
  ...
  useEffect(()=>{
    ...
    const run=async()=>{
      ...
      try{
        let s=[];
        if(tab==='hershey'){
          s=hersheyToStrokes(text,HERSHEY_FONTS[hKey]?.data||SANS_DATA,sizeMm,spacingMm,lineHMm);
        } else if(tab==='system'&&sysFont){
          // Load system font file via fetch from known paths (won't work without file) 
          // Instead show message to upload
          setErr('פונטי מערכת דורשים העלאת קובץ TTF — עבור לטאב TTF/OTF והעלה את הפונט');
          s=[];
        } else if(tab==='ttf'&&ttfBuf){
          s=await opentypeToStrokes(ttfBuf,text,sizeMm,spacingMm,lineHMm);
        }
        if(!cancelled)setStrokes(s);
      } catch(e){
        if(!cancelled){setErr('שגיאה: '+e.message);setStrokes([]);}
      }
      if(!cancelled)setLoading(false);
    };
    run();
    return()=>{cancelled=true;};
  },[text,tab,hKey,sysFont,ttfBuf,sizeMm,spacingMm,lineHMm]);
  ...
  const loadTTF=file=>{
    ...
    if(!['ttf','otf','woff'].includes(ext)){setErr('נתמך: TTF, OTF, WOFF');return;}
    const reader=new FileReader();
    reader.onload=e=>{setTtfBuf(e.target.result);setTtfName(file.name);setTab('ttf');setErr('');};
  ...
  const importFile=file=>{
    ...
    const reader=new FileReader();
    reader.onload=e=>{
      const c=e.target.result;setErr('');
      ...
      if(ext==='svg'){
        ...
        t?setText(t):setErr('לא נמצא טקסט ב-SVG — ודא שמדובר בטקסט ולא ב-path');
      } else if(ext==='dxf'){
      ...
      } else setErr(`לא נתמך: .${ext} — SVG, DXF, TXT בלבד`);
    };
  ...
  const card={background:'#fff',border:'1px solid #e2e8f0',borderRadius:10,padding:'14px 16px',marginBottom:12,boxShadow:'0 1px 3px rgba(0,0,0,0.05)'};
  ...
  const currentFontName = tab==='hershey'?HERSHEY_FONTS[hKey]?.name:tab==='system'?sysFont:ttfName;

  return(
    <div dir="rtl" style={{minHeight:'100vh',background:'#f1f5f9',fontFamily:"'Segoe UI',Arial,sans-serif",fontSize:'15px',color:'#1e293b'}}>

      <div style={{background:'#fff',borderBottom:'1px solid #e2e8f0',padding:'15px 28px',display:'flex',alignItems:'center',gap:14,boxShadow:'0 1px 4px rgba(0,0,0,0.05)'}}>
        ...
        <div>
          ...
          <div style={{fontSize:12,color:'#94a3b8',marginTop:1}}>CNC Single-Line Fonts + TTF/OTF עם opentype.js</div>
        </div>
      </div>

      <div style={{padding:'16px 22px',maxWidth:1300,margin:'0 auto',display:'grid',gridTemplateColumns:'310px 1fr',gap:16,alignItems:'start'}}>
        ...
        <div>
          ...
          <div style={card}>
            ...
            <div style={{display:'flex',gap:6,marginBottom:12}}>
              {[['hershey','CNC'],['ttf','TTF/OTF']].map(([m,l])=>(
                <button key={m} onClick={()=>setTab(m)} style={tabBtn(tab===m)}>{l}</button>
              ))}
            </div>

            {tab==='hershey'&&(
              <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:6}}>
                {Object.entries(HERSHEY_FONTS).map(([k,{name}])=>(
                  <button key={k} onClick={()=>setHKey(k)} style={{...tabBtn(hKey===k),padding:'9px 6px',fontSize:13}}>{name}</button>
                ))}
              </div>
            )}
            ...
            {tab==='ttf'&&(
              <div>
                <button onClick={()=>ttfRef.current?.click()}
                  style={{width:'100%',padding:'10px',borderRadius:7,fontSize:13,cursor:'pointer',
                    background:ttfBuf?'#ede9fe':'#f8faff',
                    border:`1px solid ${ttfBuf?'#7c3aed':'#c7d2fe'}`,
                    color:ttfBuf?'#7c3aed':'#64748b',fontFamily:'inherit',marginBottom:6}}>
                  📁 {ttfName||'העלה קובץ פונט TTF/OTF...'}
                </button>
                <input ref={ttfRef} type="file" accept=".ttf,.otf,.woff"
                  onChange={e=>{if(e.target.files[0])loadTTF(e.target.files[0])}} style={{display:'none'}}/>
                <div style={{fontSize:11,color:'#94a3b8',lineHeight:1.5}}>
                  תומך TTF, OTF, WOFF — כולל פונטים עבריים.<br/>
                  הורד פונטים עבריים בחינם: <a href="https://www.google.com/search?q=hebrew+font+ttf+free+download" target="_blank" rel="noreferrer" style={{color:'#7c3aed'}}>חיפוש Google Fonts</a>
                </div>
                {sysFonts.length>0&&(
                  <div style={{marginTop:10,borderTop:'1px solid #f1f5f9',paddingTop:10}}>
                    <div style={{fontSize:11,color:'#64748b',marginBottom:6}}>פונטים שזוהו במערכת ({sysFonts.length}):</div>
                    <div style={{fontSize:11,color:'#94a3b8',lineHeight:1.7}}>{sysFonts.join(' · ')}</div>
                    <div style={{fontSize:11,color:'#f59e0b',marginTop:4}}>⚠ להשתמש בהם — מצא את קובץ ה-TTF שלהם ב-C:\Windows\Fonts\ והעלה</div>
                  </div>
                )}
              </div>
            )}
          ...
          <div style={card}>
            ...
            <input ref={importRef} type="file" accept=".svg,.dxf,.txt"
          ...
          {/* Error */}
          {err&&<div style={{background:'#fef2f2',border:'1px solid #fecaca',borderRadius:8,padding:'10px 14px',marginBottom:12,fontSize:13,color:'#dc2626',lineHeight:1.5}}>⚠ {err}</div>}

          {/* Typography */}
          <div style={card}>
            <div style={{fontSize:12,color:'#64748b',fontWeight:600,marginBottom:8}}>📐 טיפוגרפיה</div>
            {[{l:'גודל (mm)',v:sizeMm,s:setSizeMm,min:1,max:200,step:0.5},{l:'רווח אותיות (mm)',v:spacingMm,s:setSpacingMm,min:0,max:30,step:0.5},{l:'גובה שורה (mm)',v:lineHMm,s:setLineHMm,min:5,max:300,step:1}].map(({l,v,s,min,max,step})=>(
              <div key={l} style={row}>
          ...
          <div style={card}>
            ...
            {[{l:'מהירות',v:feedRate,s:setFeedRate,c:'#2563eb'},{l:'ירידה',v:plungeRate,s:setPlungeRate,c:'#d97706'},{l:'Z בטוח',v:zSafe,s:setZSafe,c:'#2563eb'},{l:'עומק',v:zCut,s:setZCut,c:'#dc2626'},{l:'X מקור',v:ox,s:setOx,c:'#2563eb'},{l:'Y מקור',v:oy,s:setOy,c:'#2563eb'}].map(({l,v,s,c})=>(
              <div key={l} style={row}><span style={lbl}>{l} (mm):</span><input type="number" value={v} step={0.1} onChange={e=>s(+e.target.value)} style={inp(c)}/></div>
            ))}
          </div>

          {/* Stats + export */}
          <div style={{...card,background:'#f0fdf4',border:'1px solid #bbf7d0',marginBottom:10}}>
            <div style={{display:'flex',gap:16}}>
              ...
              <div><div style={{fontSize:11,color:'#6b7280'}}>פונט</div><div style={{fontSize:12,fontWeight:600,color:'#7c3aed',maxWidth:80,overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{currentFontName}</div></div>
            </div>
          </div>
          ...
          <button onClick={()=>dl(toGcode(strokes,{feedRate,plungeRate,zSafe,zCut,ox,oy}),`${fileName}.gcode`,'text/plain')}
            ...
            style={{width:'100%',padding:'12px',borderRadius:8,fontSize:14,cursor:'pointer',background:'linear-gradient(135deg,#7c3aed,#4f46e5)',border:'none',color:'#fff',fontFamily:'inherit',fontWeight:600,marginBottom:8,opacity:strokes.length&&!loading?1:0.5}}>
            {loading?'⚙ מעבד...`':'⬇ '+fileName+'.gcode'}
          </button>
          <div style={{display:'flex',gap:8}}>
            <button onClick={()=>dl(toSVG(strokes,svgColor),`${fileName}.svg`,'image/svg+xml')} disabled={!strokes.length}
        ...
        <div style={{display:'flex',flexDirection:'column',gap:12}}>
          <div style={{background:'#fff',border:'1px solid #e2e8f0',borderRadius:10,overflow:'hidden',boxShadow:'0 1px 4px rgba(0,0,0,0.05)'}}>
            <div style={{padding:'10px 16px',borderBottom:'1px solid #f1f5f9',fontSize:12,color:'#64748b',display:'flex',gap:10,alignItems:'center',background:'#f8fafc'}}>
              <span style={{fontWeight:600,color:'#1e293b'}}>תצוגה מקדימה</span>
              {currentFontName&&<span style={{background:'#ede9fe',color:'#7c3aed',padding:'2px 8px',borderRadius:4,fontSize:11}}>{currentFontName}</span>}
              {loading&&<span style={{color:'#f59e0b'}}>⚙ מעבד...</span>}
              <span style={{color:'#cbd5e1',marginRight:'auto'}}>כהה=חיתוך אדום=אוויר</span>
            </div>
          ...
          <div style={{background:'#fff',border:'1px solid #e2e8f0',borderRadius:8,padding:'10px 14px',fontSize:12,color:'#64748b',lineHeight:1.8}}>
            <span style={{fontWeight:600,color:'#374151'}}>CNC: </span>Sans · Italic · Bold · Gothic (עברית + לטינית מובנים)
            <br/><span style={{fontWeight:600,color:'#374151'}}>TTF/OTF: </span>
            כל פונט — מצא ב- <code style={{background:'#f1f5f9',padding:'1px 5px',borderRadius:3}}>C:\Windows\Fonts\</code> והעלה
          </div>
        </div>
      </div>
    </div>
